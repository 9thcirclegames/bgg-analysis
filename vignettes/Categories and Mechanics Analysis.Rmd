...
head(bgg.categories.top, how.many.categories) %>% mutate(percent.total = percent(percent.total))
```
...
And here's faceted spaghetti plot where each grey line plot the avg. (unweighted) rating for a given category in the set. As we may see, the improve in quality is widely spreaded for all the categories so far, altough I see a wider dispersion for the baseline categories.

```{r}


ggplot(aes(x = details.yearpublished, y = weight), data = left_join(bgg.cat.year, bgg.categories.rank %>%
          select(boardgamecategory, rank.segment), by = "boardgamecategory")) +
  geom_line(aes(group = boardgamecategory), color = "gray", alpha=.2) +
  geom_smooth(aes(group = 1), method = "loess" , size = 1, color = "orange", se = FALSE) +
  ylab(paste("Avg. Rating", sep=" ")) +
  xlab("Publishing Year") +
  facet_grid(. ~ rank.segment) +
  theme_bw()
